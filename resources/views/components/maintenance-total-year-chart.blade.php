...
<div
    ...
    wire:ignore
    x-data="maintenanceTotalYearChart({
        carId: @js($carId),
        endpointTemplate: @js(route('graph.maintenance.total-last-12-months', ['carId' => '__CAR_ID__'])),
    })"
    x-init="init()"
    x-on:total-cost-chart-refresh.window="refresh($event.detail)"
>
    <div class="flex items-center justify-between mb-4">
        <h3 class="text-lg font-bold text-gray-800 dark:text-gray-100">Custo total acumulado - ultimos 12 meses</h3>
    </div>
...
@once
    <script>
        ...
        function maintenanceTotalYearChart(config) {
            return {
                chart: null,
                carId: config.carId,
                ...
                requestId: 0,
                init() {
                    this.$nextTick(() => this.loadForCar(this.carId));
                },
                refresh(detail) {
                    ...
                    this.carId = carId;
                    this.$nextTick(() => this.loadForCar(carId));
                },
                extractCarId(detail) {
